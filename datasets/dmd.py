...
class DMDMTLParser(DMDParser):
    def __init__(self, root, img_list, emo_path):
        super(DMDMTLParser, self).__init__(root, img_list)
        ...
        for path in self.samples:
            ...
            cor_img_id = img_id[0:5] + "frontcropped_" + img_id[14:]

            if emo_label == -1:
                self.face_images.append("")
            else:
                img_id = img_id.split('.')[0]
                face_img_path = emo_path / 'imgs' / cor_img_id
    ...
    def __getitem__(self, index):
        ...
        return *super(DMDMTLParser, self).__getitem__(index), face_img, emo_target
